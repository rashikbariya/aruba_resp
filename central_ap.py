...
class CentralApi:
    if os.path.exists("refresh_token.yaml"):
    ...
    def get_call(self, url, header, param):
        """Generic GET call
        ...
        :rtype: Python dict
        """
        r = requests.get(self.vars["base_url"] + url, headers=header, params=param, verify=False)
        return r.json()
    ...
    def get_ap(self):
        ...
        resp = self.get_call(url, header, param=None)
        # pprint.pprint(resp)
        return resp

    def get_clients(self, param):
        """GET call for AP data
        ...
        header = {"authorization": f"Bearer {self.access_token}"}
        client_resp = self.get_call(url, header, param)
        return client_resp
